...
| `ai.js` | Monte Carlo computer player (samples unseen cards consistent with plays and voids, rolls out with a heuristic policy). Self-play A/B findings (Sep 2026, 300-game runs): more sampled worlds is the one reliable strength gain (300 vs 100 worlds won 57%); bid-aware sampling, conservative-bidding knobs, and rollout-policy refinements (Low awareness, trump-in costs, third-hand-high, draw-trump restraint) were all neutral or worse, so keep the policy simple. Self-play bidding was near an equilibrium where most auctions ended at 3 (42% set) and 4-bids were mostly blocks; since strength is indifferent, `MIN_MAKE` in ai.js gates each bid on the simulated chance of making it (70/70/50%) so the bidding feels sound to people. |
